...
try {
  ...
  sql += `  phone_change, phone_change_token, phone_change_sent_at, confirmed_at, email_change_sent_at,\n`;
  ...
  const userValues = users.map(u => {
    ...
    return `  ('00000000-0000-0000-0000-000000000000', '${id}', 'authenticated', 'authenticated', '${email}', '${encrypted_password}', ${email_confirmed_at}, ${raw_user_meta_data}, ${raw_app_meta_data}, ${created_at}, ${created_at}, '', '', '', '', false, NULL, NULL, '', '', NULL, ${email_confirmed_at}, NULL, false, NULL)`;
  });
  ...
  sql += `  id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at\n`;
  sql += `)\nVALUES\n`;
  ...
  const identityValues = users.map(u => {
    ...
    const created_at = u.created_at ? `'${u.created_at}'` : 'NOW()';
    return `  ('${id}', '${id}', '${identity_data.replace(/'/g, "''")}', 'email', ${created_at}, ${created_at}, ${created_at})`;
  });
